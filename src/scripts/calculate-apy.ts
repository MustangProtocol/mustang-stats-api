...
async function calculateAndUpdateBranchApy() {
  ...
  try {
    ...
    for (const branch of branches) {
      ...
      const apyResultCurrent = apyResult7d;
      const apyCurrentPercentage = formatApyAsPercentage(apyResultCurrent.apy);
      ...
      console.log(`  └─ Current APY (7-day avg): ${apyCurrentPercentage}%`);
      console.log('');
